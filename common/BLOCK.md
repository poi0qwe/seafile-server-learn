...
- ### 块操作
    ...
    |释放块的句柄|释放句柄空间。|
    |遍历块|给定store_id和version，并提供用户函数和用户参数，去遍历存储中的所有块。<br/>每访问一个块，向用户函数传入store_id、version、block_id以及用户参数。|
    |复制块|提供块A和块B的store_id、version、block_id，将A的内容复制到B；<br/>若不存在B则创建。|
    |移除存储|移除存储中的所有块。|

    （“移除存储”需要仓库的seafile版本为1）
...
- ### 块操作的实现
    ...
    |释放块的句柄|释放结构体空间。|
    |遍历块|合成存储路径，遍历文件名得到block_id，随后呈递给用户函数。|
